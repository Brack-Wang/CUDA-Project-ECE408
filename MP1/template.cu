...
int main(int argc, char **argv) {
  ...
  int deviceSize = inputLength * sizeof(float);
  ...
  int blockPerGrid = ceil(inputLength/256.0);
  int threadPerBlock = 256;

  wbTime_start(Compute, "Performing CUDA computation");
  //@@ Launch the GPU Kernel here
  vecAdd<<<blockPerGrid, threadPerBlock>>>(deviceInput1, deviceInput2, deviceOutput, inputLength);
