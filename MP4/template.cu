    ...
    for (int i = 0; i < MASK_WIDTH; i++) {
      for (int j = 0; j < MASK_WIDTH; j++) {
        for (int k = 0; k < MASK_WIDTH; k++) {
          out_value += M[i][j][k] * N_ds[i+tz][j+ty][k+tx];
        }
      }
    }
    if (z_o < z_size && y_o < y_size && x_o < x_size) {
      output[z_o * y_size * x_size + y_o * x_size + x_o] = out_value;
    }
  }
  __syncthreads ();
}
...
int main(int argc, char *argv[]) {
  ...
  int size_i = (inputLength - 3) * sizeof(float);
  int size_o = (inputLength - 3) * sizeof(float);
  cudaMalloc((void **) &deviceInput, size_i);
  ...
  cudaMemcpy(deviceInput, hostInputValue, size_i, cudaMemcpyHostToDevice);
  cudaMemcpyToSymbol(M, hostKernel, MASK_WIDTH*MASK_WIDTH*MASK_WIDTH*sizeof(float));
  wbTime_stop(Copy, "Copying data to the GPU");

  wbTime_start(Compute, "Doing the computation on the GPU");
  //@@ Initialize grid and block dimensions here
  dim3 blockPerGrid(ceil(x_size/(1.0*TILE_WIDTH)), ceil(y_size/(1.0*TILE_WIDTH)), ceil(z_size/(1.0*TILE_WIDTH)));
  dim3 threadPerBlock(TILE_WIDTH + MASK_WIDTH - 1,TILE_WIDTH + MASK_WIDTH - 1,TILE_WIDTH + MASK_WIDTH - 1);

  //@@ Launch the GPU kernel here
  conv3d<<<blockPerGrid, threadPerBlock>>>(deviceInput, deviceOutput, z_size, y_size, x_size);
  ...
  float hostOutpuValue[inputLength - 3];
  cudaMemcpy(hostOutpuValue, deviceOutput, z_size * y_size * x_size * sizeof(float), cudaMemcpyDeviceToHost);
  for (int k = 3; k < inputLength; k++) {
    hostOutput[k] = hostOutpuValue[k - 3];
  }
